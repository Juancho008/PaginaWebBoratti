...
export const cacheScriptPath = join(rootDir, 'src', 'data', 'instagram-posts.js');
export const assetsDir = join(rootDir, 'src', 'assets', 'instagram');
...
export async function storePostThumbnailsLocally(posts) {
  await mkdir(assetsDir, { recursive: true });

  const storedPosts = [];

  for (const post of posts) {
    const remoteUrl = post.thumbnail_url || post.media_url;
    ...
    if (!response.ok) {
      throw new Error(`No se pudo descargar la miniatura de ${post.id}: ${response.status}`);
    }

    const filename = `${post.id}.jpg`;
    const filePath = join(assetsDir, filename);
    const localUrl = `./assets/instagram/${filename}`;
...
export async function saveCachedPosts(posts) {
  ...
  const postsWithLocalImages = await storePostThumbnailsLocally(normalizedPosts);
  const payload = {
  ...
  await writeFile(
    cacheScriptPath,
    `window.__INSTAGRAM_FEED__ = ${JSON.stringify(payload, null, 2)};\n`,
    'utf8'
  );
  return payload;
}
